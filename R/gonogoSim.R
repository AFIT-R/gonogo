...
#' @param n2 Phase II test size, \eqn{n2 \ge 0}, \eqn{n2 = 0} skips Phase II and proceeds directly to Phase III
...
gonogoSim <- function(mlo, mhi, sg, n2, n3, p, lam, dm = 0, ds = 0, ln = F, plt = 0, neyer = F, iseed = -1, IIgo = T, M = 1)
{
  ...
  if (!neyer)
  {
  	...
  	if (IIgo)
  	{
    	...
    	{
    	  w = spIIIsim(d0, dat0, tmu, tsig, n3, p, lam, reso, ln, iseed);
    	  d0 = w[[1]];
    	  dat0 = w[[2]];
    	  jvec = w[[3]];
    	}

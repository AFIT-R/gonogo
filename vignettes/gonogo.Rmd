...
The third entry is $\lambda$, which is the RMJ skewedness coefficient. For values of $p$ near 0, a large $\lambda$ is recommended since this forces an upward bias where bigger upward steps are taken increasing the probability of a response ($R = 1$). For $p$ near 1, a small $\lambda$ is recommended since this forces a downward bias where bigger downward steps are taken increasing the probability of a non-response ($R = 1$).
...
__ *11.71060__ was used to begin Phase III with the paper's value. The Gonogo recommendation of 11.71214 is the value we would recommend here.   
